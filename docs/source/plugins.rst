...
- ``{_home}`` - path to Nexus's home/working directory
...
4. The command will save the song text into a file named with the song's title in the **Nexus** working directory.

**Example tutorial command**

**Nexus** provides simple tutorial command to show how it work, to run it just ask model for execute ``tutorial test command`` and it will show you how it works:
...
- stdio: best for local processes started by Nexus; no network involved.
...
- ``Docker image name`` *image_name*
  ...
  Name of the Docker image used by the sandbox. Default: ``nexus_system``

- ``Docker container name`` *container_name*

  Name of the Docker container started for the sandbox. Default: ``nexus_system_container``
...
Then, copy the following two items into **Nexus**:
...
- ``User-Agent`` *user_agent*
  ...
  Custom User-Agent string for requests. *Default:* `nexus-client-wikipedia-plugin/1.0 (+https://otecta.discourse.group/)`
